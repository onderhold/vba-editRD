...
class TestCLIOptionsDebugging:
    ...
    def write_options_file(self, tmp_path: Path, test_name: str, options_dict: dict) -> Path:
        ...
        output_options = {k: v for k, v in options_dict.items() if k.startswith(('stdout_', 'stderr_', 'output_', 'return_'))}
        other_options = {k: v for k, v in options_dict.items() 
                        if not any(k.startswith(prefix) for prefix in 
                                 ['config_', 'original_', 'template_', 'resolved_', 'expected_', 'actual_', 'cli_', 'stdout_', 'stderr_', 'output_', 'return_'])}
...
"""
        ...
        if result.returncode == 0:
            ...
            if created_dirs:
                ...
                updated_options_file = self.write_options_file(tmp_path, f"real_doc_updated_{vba_app}", effective_options)
